...
def pullLast24Hours():
	query = '@APGradCountdown'
	max_tweets = 1000
	tweets = [tweet for tweet in tweepy.Cursor(api.search, q=query).items(max_tweets)]
	return [tweet for tweet in tweets if (datetime.now() - tweet.created_at).days < 1]
...
# This just calculates the days until the selected graduation. Keeping things DRY
def days_until(current_day, current_year, grad_day, grad_year):
	return str((grad_year - current_year)*365 + (grad_day-current_day))



# This creates the traditional daily tweet which includes actual graduation days.
def createDailyTweet(day, year, graduations):
	tweet_lines = []

	days_until_closest = days_until(
						day, year,
						int(graduations[0][0]),int(graduations[0][1]))
	...
	if (days_until_closest == '0'):
		...
		tweet_lines.append(days_until_closest + " days until the " + graduations[0][1]
							+ " " + graduations[0][2] + "!")
		...
		days_until_rand = days_until(
						day, year,
						int(graduations[rand][0]),int(graduations[rand][1]))

		tweet_lines.append(days_until_rand + " days until the " + graduations[rand][1]
							+ " " + graduations[rand][2] + "!")
...
# This first collects 1000 tweets from the last 24 hours and checks to see if any
# match the criterion for a response. If they tweet with the following format
# @APGradCountDown season yyyy      then a response will be made with how many days
# they have until they graduate.
def respond(day, year, graduations):
	...
	for tweet in latest_tweets:
		...
		if season == 'spring' or season == 'winter' or season == 'summer':
			if (status_year > max_year and season != 'spring'):
				response = response + 'You\'ve got a long way to go!'
			elif (status_year < min_year):
				response = response + 'You already graduated!'
			else:
				for graduation in graduations:
					if graduation[1] == status_year and graduation[2].split()[0].lower() == season:
						days_until_grad = days_until(
										day, year,
										int(graduation[0]),int(graduation[1]))
				if days_until_grad == '': 
					response = response + 'Your graduation is not on record. Sorry!'
				else:
					response = response + "You graduate in " + days_until_grad + " days!"
